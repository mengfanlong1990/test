...
#import "MFLPhotoPreviewer.h"
#import "UIView+MFLExtension.h"
#import <MBProgressHUD/MBProgressHUD.h>
@interface MFLPhotoPreviewer () <UIScrollViewDelegate>
...
@implementation MFLPhotoPreviewer
...
- (void)previewFromImageView:(UIImageView *)fromImageView inContainer:(UIView *)container highDefinitionImage:(UIImage *)highDefinitionImage
{
    ...
    if (highDefinitionImage) {
        image = highDefinitionImage;
    }else{
        image= fromImageView.image;
    }
...
- (void)longPress:(UILongPressGestureRecognizer *)recognizer {
    ...
    if (recognizer.state == UIGestureRecognizerStateBegan) {
        UIAlertController *alertController = [UIAlertController alertControllerWithTitle:@"保存头像?" message:nil preferredStyle:UIAlertControllerStyleActionSheet];
        ...
        [alertController addAction:[UIAlertAction actionWithTitle:@"保存" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
            ...
            UIImageWriteToSavedPhotosAlbum(self.imageView.image,self,@selector(imageSavedToPhotosAlbum:didFinishSavingWithError:contextInfo:),NULL); // 写入相册
...

{
    ...
    if(!error){
        [MBProgressHUD showHUDOnlyText:@"保存成功！" afterDelay:2.0];
    }else{
        NSLog(@"savefailed");
    }
